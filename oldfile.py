...
# python
def main():
    # Prompt for comma-separated modulation types (e.g. "QPSK,16QAM")
    mod_input = input("Enter modulation types separated by comma (Default: QPSK): ") or "QPSK"
    mod_list = [m.strip().upper() for m in mod_input.split(",") if m.strip()]

    # mapping from string to M
    mod_dic = {"BPSK": 2, "QPSK": 4, "8PSK": 8, "16QAM": 16, "64QAM": 64}

    # Basic common parameters (can still be overridden per-mod if desired)
    subc = 1024
    cp = subc // 4
    L = 4  # oversampling factor used in several places
    CR_list = [10 ** (1/10), 10 ** (3/10), 10 ** (5/10), 10 ** (7/10)]
    CR_list = [10 ** (1/20), 10 ** (3/20), 10 ** (5/20), 10 ** (7/20)]
    CR_list = [0.8, 1, 1.2, 1.4, 1.6]

    for mod_str in mod_list:
        if mod_str not in mod_dic:
            print(f"Skipping unknown modulation: {mod_str}")
            continue

        M = mod_dic[mod_str]
        bits_per_symbol = int(np.log2(M))

        # ask number of OFDM symbols once per modulation
        num_symb = int(input(f"Enter number of OFDM symbols for {mod_str} (Default: 100): ") or "100")
        ...
        for EbNo_dB in p["EbNo_range"]:
            ...
            for CR in CR_list:
                BER_results[f'clipped_CR={CR}'].append(bit_error_clipped[CR] / total_bits)
                BER_results[f'clipped_filtered_CR={CR}'].append(bit_error_filtered[CR] / total_bits)

            print(f"  {mod_str} | Eb/No: {EbNo_dB:.2f} dB | BER (No clip): {ber_no_clip:.6f} | BER (Theory): {ber_theory:.6f}")

        # Plot BER curves for this modulation
        ...
        # PAPR simulation (keeps existing logic, run per modulation)
        calc_papr = 'y'
        if calc_papr == 'y' or calc_papr == '':
            L_values = [4]
            cr_db_list = [20 * np.log10(c) for c in CR_list]
            samples_per_L = 1000
            print(f"Calculating PAPR for {samples_per_L} blocks/L for {mod_str} with CRs (dB) = {cr_db_list} and linear = {CR_list}")
            ...
            for L in L_values:
                ...
                plot_CCDF_compare(papr_dict, subc, title_suffix=f' - {mod_str}')

            print(f"PAPR simulation done for {mod_str}.")
        else:
            print("Terminating PAPR step.")
