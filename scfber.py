...
for EbNo_dB in EbNo_range:
    ...
    # for _ in range(1):
        ...
        if (EbNo_dB == EbNo_range[0] or EbNo_dB == EbNo_range[-1]) and not plotted:
            # plot_constellation(rx_symbols_clipped_scf, mod, limit, title=f'Received Constellation at E$_b$/N$_0$ = {EbNo_dB}dB (Clipped)', label='Rx Symbols (Clipped)', color='magenta', save=(constel_image_path+f"Rx_clipped_scf_{EbNo_dB}db.png"))
            plot_constellation(rx_symbols_scf, mod, limit, title=f'Received Constellation at E$_b$/N$_0$ = {EbNo_dB}dB (Filtered)', label='Rx Symbols (Filtered)', color='blue', save=(constel_image_path+f"Rx_scf_{EbNo_dB}db.png"))
